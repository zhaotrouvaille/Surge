...
const apiUrl = "校园网登录地址";
const authUrl = apiUrl + "?method=login";
const logoutUrl = apiUrl + "?method=logout";
const getQueryUrl = "http://123.123.123.123/";
const getUserInfoUrl = apiUrl + "?method=getOnlineUserInfo";
...
async function is_authed() {
    try {
        ...
        if (resp.statusCode !== 200) {
            ...
            try {
                const json_data = JSON.parse(resp.body);
                return json_data.result !== "fail";
            } catch (e) {
                $.log("❌ JSON解析失败：", e);
                return false;
            }
...
async function get_query_str() {
    try {
        ...
        if (resp.statusCode !== 200) {
            $.log(`❌ 获取query信息失败: 状态码 ${resp.statusCode}`);
            return false;
        }
        const pattern = /href='(.*)'/;
        ...
        if (match) {
            ...
            $.log(`🟡 获取到的query字符串: ${url.search.slice(1)}`);
...
async function do_auth() {
    const services = ["校园网", "中国电信"];
    for (let service of services) {
        const query_str = await get_query_str();
        if (!query_str) {
            $.log("❌ 无法获取query字符串，认证失败");
            continue;
        }
        ...
        try {
            const data = {
                ...
                queryString: encodeURIComponent(query_str),
                operatorPwd: "",
                operatorUserId: "",
                validcode: "",
            ...
            if (resp.statusCode === 200) {
                try {
                    ...
                    if (result.result === "success") {
                        $.log(`🎉 认证成功，使用服务: ${service}`);
                        notice += `🎉 认证成功，使用服务: ${service}\n`;
                        return true;
                    } else {
                        const error_message = result.message || '未知错误';
                        $.log(`❌ 使用服务 ${service} 认证失败: ${error_message}`);
                        notice += `❌ 使用服务 ${service} 认证失败: ${error_message}\n`;
                    }
                } catch (e) {
                    $.log(`❌ JSON解析失败：${e}`);
                }
            ...
            $.log(`❌ 使用服务 ${service} 认证失败： ${e}`);
            notice += `❌ 使用服务 ${service} 认证失败： ${e}\n`;
        }
    }
...
async function do_logout() {
    try {
        const resp = await $.http.get(logoutUrl);
        if (resp.statusCode === 200) {
            const result = JSON.parse(resp.body);
            if (result.result === "success") {
                $.log("🎉 退出登录成功");
                ...
                $.log("❌ 退出登录失败: " + result.message);
                notice += `❌ 退出登录失败: ${result.message}\n`;
                return false;
            }
        } else {
            $.log("❌ 退出登录失败: 网络错误");
...
(async () => {
    try {
        let wifiMatched = false;
        for (let i in wifi_list) {
            if (wifi_list[i] === ssid) {
                wifiMatched = true;
                await auth_job();
                break;
        ...
        if (!wifiMatched) {
            $.log("🟡 当前WiFi不在认证列表中，跳过认证");
            //notice += "🟡 当前WiFi不在认证列表中，跳过认证\n";
        }
        $.log("✅ 脚本执行完成");
        // 只有在进行认证操作时才发送通知
        if(notice){
